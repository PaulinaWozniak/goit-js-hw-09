...
const options = {
    ...
    onClose (selectedDates) {
        ...
        if (selectedDates[0] <= options.defaultDate) {
            ...
            return (0, _notiflix.Notify).failure("Please choose a date in the future");
        } else {
...
let date = (0, _flatpickrDefault.default)(picker, options);
...
startBtn.addEventListener("click", ()=>{
    ...
    const timer = setInterval(()=>{
        ...
        if (selectedInMs - Date.now() > 0) timeContext(remainingMs);
        else {
            clearInterval(timer);
            (0, _notiflix.Notify).success("Time is up!");
            startBtn.disabled = false;
...
function parseDate(date1) {
    return selectedInMs = Date.parse(date1);
}
